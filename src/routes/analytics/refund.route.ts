...
import { getRefundAnalytics } from '../../analytics/refund';

const router = Router();

/**
 * GET /api/analytics/refund
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const analytics = await getRefundAnalytics(start, end);
    ...
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to fetch refund analytics',
    });
  }
});
